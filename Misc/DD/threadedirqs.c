/* 4 bottom half mechanism available in linux to server interrupts
 * workqueue
 * THREADED IRQS
 * softirqs
 * tasklet
 *
 * Threaded IRQs
 * Register the handler and Linux handles the scheduling part
 * main aim of this mechanism is to reduce time spent with
 * interrupts being disabled and that will increase chances of
 * handling other interrupts
 *
 * Once interrupt hits , we will disable all the interrupts and
 * process our data or something. Once processed then re-enable
 * if we take more time in processing ISR the we might miss the
 * disabled interrupts in the ISR
 * To solve this, we do important process in ISR and defer other
 * stuff to the thread function. So, we respond to other interrupts
 *
 * API
 * int request_threaded_irq(unsigned int irq,
 * 			irq_handler_t handler,   //primary handler if NULL then irq_default_primary_handler 
 * 			irq_handler_t thread_fn, //if primary handler returns IRQ_WAKE_THREAD then called 
 * 			unsigned long irqflags,  //irq flags:IRQF_SAMPLE_RANDOM,IRQF_DISABLED,IRQF_TIMER,IRQF_SHARED etc.
 * 			const char *devname,     //device name
 * 			void *dev_id);           //unique cookie from device id to remove IRQ from shared int. line
 * if threaded IRQ handler takes more time when ISR handler takes
 * less time. In this case, we miss the thread function
 * instead of interrupt. Since, thread function is busy.
 * 
 * */
...
/* softirq handler */
static void gpio_interrupt_thread_fn(int irq, void *dev_id) {
...
static int    __init hwm_driver_init(void) 
{
  ...
  if (request_threaded_irq(GPIO_irqNumber,         // IRQ number
		  ...
		  gpio_irq_thread_fn,              // irq thread handler (bottom half)
